...
<script type="text/javascript" src="<%=basePath%>member/script/jquery-1.9.0.min.js"></script>
<script type="text/javascript" src="<%=basePath%>member/script/login.js"></script>
<link href="<%=basePath%>member/css/login2.css" rel="stylesheet" type="text/css">
		...
		if($("#user").val().length!=0){
			...
			var url="<%=basePath%>servlet/MemberServlet?method=isSameNo";
				 ...
				 $.post(url,{"no":$("#user").val()},function(result){
					 if(result=="-1"){
						  $("#userCue").html("<font color=\"red\"><b>√可用！</b></font>");
					  }else{
					 	 $("#userCue").html("<font color=\"red\"><b>×已被注册！</b></font>");
					  }
				  });
		}
	}
					...
					<div class="login_form">
						<form action="<%=basePath%>servlet/LoginServlet"
							...
							<div class="shenFenArea" id="shenFenArea">
								<label style="margin-top: -13px;margin-left: 15px" class="input-tips" for="u">身份：</label>
								<div  style="margin-top: 20px" id="uArea">
									<label> <input type="radio" name="identity"
										value="1" id="RadioGroup1_0" checked="checked"/> 租赁会员</label> <label> <input
										type="radio" name="identity" value="2" id="RadioGroup1_1" />
										营业门店</label> <label> <input type="radio" name="identity"
										value="3" id="RadioGroup1_1" /> 企业人员</label>
								</div>
			...
			<div class="web_login">
				<form name="form2" id="regUser" accept-charset="utf-8"
					...
					<ul class="reg_form" id="reg-ul">
						...
						<li><label for="realName " class="input-tips2">真实姓名：</label>
							<div class="inputOuter2">
